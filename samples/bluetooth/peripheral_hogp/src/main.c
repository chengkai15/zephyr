...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	/* The HOGP Device requests an encrypted link itself, see
	 * CONFIG_BT_HOGP_DEVICE_SECURITY_REQUEST.
	 */
}
			     ...
			     uint16_t buf_size)
{
	...
	if (report_id == REPORT_ID_MOUSE && report_type == BT_HID_REPORT_TYPE_INPUT) {
...
static const struct bt_hids_cb hogp_cb = {
...
static void send_mouse_report(void)
{
	...
	err = bt_hogp_device_send_report(NULL, REPORT_ID_MOUSE, report, sizeof(report),
					 NULL, NULL);
	if (err && err != -ENOTCONN) {
