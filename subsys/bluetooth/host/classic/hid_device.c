...
static int hid_set_idle_handle(struct bt_hid_device *hid, struct net_buf *buf, uint8_t param);
...
{
	struct bt_conn *ctrl_conn = hid_device_conn.ctrl_session.br_chan.chan.conn;

	return (ctrl_conn != NULL) && (ctrl_conn != conn);
}
...
static int hid_send_handshake(struct bt_hid_session *session, uint8_t result_code)
{
	struct net_buf *buf;
...
static int hid_control_handle(struct bt_hid_device *hid, struct net_buf *buf, uint8_t control)
{
	...
	case BT_HID_PAR_CONTROL_VIRTUAL_CABLE_UNPLUG:
		...
		bt_br_unpair(bt_conn_get_dst_br(bt_hid_device_conn(hid)));
...
static const struct {
	...
	{BT_HID_MSG_TYPE_SET_IDLE, hid_set_idle_handle},
};
...
static void bt_hid_l2cap_ctrl_connected(struct bt_l2cap_chan *chan)
{
	...
	err = bt_l2cap_chan_connect(bt_hid_device_conn(hid), &hid->intr_session.br_chan.chan,
...
static void bt_hid_l2cap_intr_connected(struct bt_l2cap_chan *chan)
{
	...
	if (hid->ctrl_session.br_chan.chan.conn == NULL) {
		LOG_WRN("HID: CTRL not connected, reject INTR");
...
static int bt_hid_l2cap_ctrl_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(hid_ctrl_handlers); i++) {
		if (hid_ctrl_handlers[i].type == type) {
			hid_ctrl_handlers[i].handler(hid, buf, param);
			return 0;
		}
	}
...
int bt_hid_device_unregister(void)
{
	...
	if (hid_device_conn.state != BT_HID_STATE_DISCONNECTED) {
		if (hid_device_conn.intr_session.br_chan.chan.conn != NULL) {
			bt_l2cap_chan_disconnect(&hid_device_conn.intr_session.br_chan.chan);
		}

		if (hid_device_conn.ctrl_session.br_chan.chan.conn != NULL) {
...
int bt_hid_device_connect(struct bt_conn *conn, struct bt_hid_device **hid_out)
{
	...
	bt_hid_session_init(hid, BT_HID_ROLE_INITIATOR);
...
int bt_hid_device_disconnect(struct bt_hid_device *hid)
{
	...
	if (hid->intr_session.br_chan.chan.conn != NULL) {
		bt_l2cap_chan_disconnect(&hid->intr_session.br_chan.chan);
	} else if (hid->ctrl_session.br_chan.chan.conn != NULL) {
		bt_l2cap_chan_disconnect(&hid->ctrl_session.br_chan.chan);
	}
...
int bt_hid_device_get_report_response(struct bt_hid_device *hid, uint8_t type, struct net_buf *buf)
{
	...
	if (type < BT_HID_REPORT_TYPE_INPUT || type > BT_HID_REPORT_TYPE_FEATURE) {
		return -EINVAL;
	}
	...
	memset(hdr, 0, sizeof(struct bt_hid_hdr));
	hdr->header = BT_HID_BUILD_HDR(BT_HID_MSG_TYPE_DATA, type);
...
int bt_hid_device_send_input_report(struct bt_hid_device *hid, struct net_buf *buf)
{
	struct bt_hid_hdr *hdr;
	int err;
